...
local S = {
    ...
    Crosshair   = false,
    -- Combat extras
    AutoShot    = false,   -- auto-shoot murderer when you have the gun
    SilentAim   = false,   -- redirect all bullets to murderer regardless of aim
    -- Player
    Speed       = 16,
    Jump        = 50,
    InfJump     = false,
...
-- ============================================================
--  ROLE DETECTION  (multi-layer: attributes, backpack, tool, values)
-- ============================================================
local RoleCache2 = {}   -- persists across frames so billboard shows role without tool

local function getRole(p)
    if p == LP then return "Self" end
    local c = p.Character if not c then return RoleCache2[p] or "Unknown" end

    -- 1) Player / Character attributes (MM2 often sets these server-side)
    for _, obj in ipairs({p, c}) do
        for _, attr in ipairs({"Role","role","PlayerRole","Rank"}) do
            local v = pcall(function() return obj:GetAttribute(attr) end) and obj:GetAttribute(attr)
            if v then
                local s = tostring(v):lower()
                if s:find("murder") then RoleCache2[p]="Murder"  return "Murder"  end
                if s:find("sheriff") then RoleCache2[p]="Sheriff" return "Sheriff" end
                if s:find("innocent") then RoleCache2[p]="Innocent" return "Innocent" end
            end
        end
    end

    -- 2) StringValues / IntValues inside character
    for _, v in ipairs(c:GetChildren()) do
        if v:IsA("StringValue") or v:IsA("IntValue") then
            local name = v.Name:lower()
            local val  = tostring(v.Value):lower()
            if name:find("role") or name:find("type") or name:find("team") then
                if val:find("murder") then RoleCache2[p]="Murder"  return "Murder"  end
                if val:find("sheriff") then RoleCache2[p]="Sheriff" return "Sheriff" end
            end
        end
    end
    ...
    if tool then
        ...
        if n:find("knife") or n:find("shard") or n:find("dagger") or n:find("blade") then
            RoleCache2[p]="Murder"; return "Murder"
        end
        if n:find("gun") or n:find("sheriff") or n:find("revolv") or n:find("pistol") then
            RoleCache2[p]="Sheriff"; return "Sheriff"
        end
    end

    -- 4) Backpack (holding weapon but not yet equipped)
    ...
    -- 5) Check game data folders in ReplicatedStorage
    for _, folder in ipairs(ReplicatedStorage:GetChildren()) do
        local fn = folder.Name:lower()
        if fn:find("data") or fn:find("game") or fn:find("role") or fn:find("player") then
            local pf = folder:FindFirstChild(p.Name) or folder:FindFirstChild(tostring(p.UserId))
            if pf then
                for _, v in ipairs(pf:GetChildren()) do
                    if v.Name:lower():find("role") then
    ...
    -- Return cached role if we had one (e.g. player dropped knife temporarily)
    if RoleCache2[p] and RoleCache2[p] ~= "Innocent" then
        return RoleCache2[p]
    end

    RoleCache2[p] = "Innocent"
    return "Innocent"
end

-- Clear cache when character resets
Players.PlayerAdded:Connect(function(p)
    p.CharacterAdded:Connect(function() RoleCache2[p]=nil end)
end)
Players.PlayerRemoving:Connect(function(p) RoleCache2[p]=nil end)
...
-- ============================================================
--  AUTO SHOT MURDER  (fires gun at murderer automatically)
-- ============================================================
local _shotCooldown = 0

local function getMurderTarget()
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP then
            local r = getRole(p)
            if r == "Murder" then
...
local function tryAutoShot()
    if not S.AutoShot then return end
    if tick() - _shotCooldown < 0.9 then return end

    -- Only fire if local player holds a gun
    local tool = Char and Char:FindFirstChildOfClass("Tool")
    if not tool then return end
    local tn = tool.Name:lower()
    if not (tn:find("gun") or tn:find("sheriff") or tn:find("revolv") or tn:find("pistol")) then return end

    local _, murderHead = getMurderTarget()
    if not murderHead then return end

    _shotCooldown = tick()

    -- Try firing all remote events inside the gun tool
    for _, rem in ipairs(tool:GetDescendants()) do
        if rem:IsA("RemoteEvent") then
            local n = rem.Name:lower()
            if n:find("shoot") or n:find("fire") or n:find("shot") or n:find("bullet") or n:find("click") then
                pcall(function() rem:FireServer(murderHead.CFrame, murderHead.Position) end)
            end
        end
    end

    -- Also try top-level ReplicatedStorage shoot remotes
    for _, rem in ipairs(ReplicatedStorage:GetDescendants()) do
        if rem:IsA("RemoteEvent") then
            local n = rem.Name:lower()
            if n:find("shoot") or n:find("fire") or n:find("gunshot") then
                pcall(function() rem:FireServer(murderHead.CFrame, murderHead.Position) end)
            end
        end
    end

    -- Move crosshair to murder so the local fire logic also hits
    Cam.CFrame = CFrame.lookAt(Cam.CFrame.Position, murderHead.Position)
end

-- ============================================================
--  SILENT AIM  (redirect bullets to murderer via metatable hook)
-- ============================================================
...
local function hookSilentAim()
    ...
    -- Only hook if executor exposes getrawmetatable / setreadonly
    local ok = pcall(function()
        ...
        mt.__namecall = newcclosure(function(self, ...)
            ...
            -- Intercept FireServer calls on RemoteEvents that look like shoot events
            if method == "FireServer" and S.SilentAim then
                local ok2, name = pcall(function() return self.Name:lower() end)
                if ok2 and (name:find("shoot") or name:find("fire") or name:find("shot") or name:find("bullet")) then
                    local _, murderHead = getMurderTarget()
                    if murderHead then
                        local args = {...}
                        -- Replace any Vector3 / CFrame / BasePart arg with murder's head
                        for i, v in ipairs(args) do
                            if typeof(v) == "Vector3" then
                                args[i] = murderHead.Position
                            elseif typeof(v) == "CFrame" then
                                args[i] = murderHead.CFrame
                            elseif typeof(v) == "Instance" and pcall(function() return v:IsA("BasePart") end) and v:IsA("BasePart") then
                                args[i] = murderHead
                            end
                        end
    ...
    if not ok then
        warn("[Phantom] Silent Aim: executor does not support metatable hooks — feature unavailable")
    end
end
...
do local p=Pages["Combat"]
    ...
    addSeparator(p)
    addToggle(p,"Auto Shoot Murderer",  function() return S.AutoShot  end, function(v) S.AutoShot=v end)
    addToggle(p,"Silent Aim",           function() return S.SilentAim end, function(v)
        S.SilentAim=v
